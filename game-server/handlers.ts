...
export function getStartTurnsHandler(io: Server<events.ClientEvents, events.ServerEvents>): RequestHandler {
...
function completeTurn(currentPlayer: Player, card: number, io: Server<events.ClientEvents, events.ServerEvents>) {
...
export function findWinningCard(): CardPlayed {
...
function NewCard(card: number): Card {
    if (card > 51) {
        return {
            value: card,
...
function NewCardPlayed(card: number, player: Player): CardPlayed {
    return {
        card: NewCard(card),
        player: player,
    };
